...
function webaction(main) {
    ...
    return actionWrapper(async (params={}) => {
        // Asset Compute Throttling Service metrics
        // for quick feedback loop, store predicted run duration next to actual run duration
        if (params && params.metrics && params.predictedRunDuration) {
            console.log('params.metrics and params.predictedRunDuration are available');
            params.metrics.add({
