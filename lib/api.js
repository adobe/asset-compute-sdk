...
async function createTransformerFromWorker(manifestFile = "./pipeline-manifest.json", renditionCallback) {
    const rawManifest = await fs.readJson(manifestFile);
    
    return new WorkerCallbackTransformer("worker", renditionCallback, rawManifest);
}
