...
function batchWorker(renditionsCallback, options = {}) {
    ...
    return actionWrapper(async function (params) {
        ...
        if(usePipeline){
             ...
             * options example:
             * {
                supportedRenditionFormats: supported_formats_Array,
...
function shellScriptWorker(script = "worker.sh", options = {}) {
    ...
    return actionWrapper(function (params) {
      ...
      if(usePipeline){
        ...
        const shellscriptWorkerManifestName = "pipeline-manifest.json";
        if (fs.existsSync(shellscriptWorkerManifestName)) {
          console.log("Reading shellscript worker manifest");
          try{
            const manifest = fs.readJsonSync(shellscriptWorkerManifestName);
            options.manifest = manifest;
          } catch (e) {
            console.log("Error reading shellscript worker manifest: ", e);
            throw new Error(`Shell script manifest pipeline-manifest.json could  not be read, worker cannot be used as part of a pipeline`);
          }
        } else {
          throw new Error(`Shell script manifest pipeline-manifest.json not found, worker cannot be used as part of a pipeline`);
        }
