...
async function createTransformerFromWorker(renditionCallback, options={}) {
    ...
    manifests.forEach(oneManifest => {
        const workerName = `workerCallback-`;
        const oneTransformer = new WorkerCallbackTransformer(renditionCallback, oneManifest, workerName);
        debug(`Created a transformer named ${workerName} with manifest:`);
        debug(oneManifest);
