...
class TemporaryCloudStorage {
    constructor(params){
        this.aioLibFiles = null;

        // optional authorization passed in for using specific azure blob storage instead of ootb storage
        if (params && !isBlankString(params.azureStorageAccount)
            && !isBlankString(params.azureStorageAccessKey)
            && !isBlankString(params.azureContainerName)) {
            this.azureAuth = {
                azure: { 
                    storageAccount: params.azureStorageAccount,
                    storageAccessKey: params.azureStorageAccessKey,
                    containerName: params.azureContainerName
                }
            };
        }
    }
    ...
    async _init() {
        // optional authorization passed in for using specific azure blob storage instead of ootb storage
        if (this.azureAuth) {
            console.log("Using custom azure blob storage for ('aio-lib-files')");
            this.aioLibFiles = await filesLib.init(this.azureAuth);
        } else {
