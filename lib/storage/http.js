...
async function download(asset, file) {
    try {
        const contentType = mime.contentType(asset.name) || DEFAULT_CONTENT_TYPE;
        console.log(`downloading source: ${AssetComputeLogUtils.redactUrl(asset.url)} into ${file}`);

        //asset.headers = Object.assign({"content-type": contentType}, asset.headers);
