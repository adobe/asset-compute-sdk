...
const { AssetComputeLogUtils, GenericError, RenditionTooLarge } = require('@nui/asset-compute-commons');
...
async function upload(rendition) {
    ...
    try {
        const contentType = mime.contentType(path.extname(file)) || DEFAULT_CONTENT_TYPE;
        if (typeof target === 'string') {
            console.log(`uploading rendition ${file} to ${AssetComputeLogUtils.redactUrl(target)}`);
            ...
            console.log(`uploading rendition ${file} as multi-part to ${AssetComputeLogUtils.redactUrl(target.urls[0])} and ${target.urls.length-1} more urls`);
