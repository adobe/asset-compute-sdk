 ...
 * @param {String} Path to local file system, where file is downloaded
...
async function getPreSignedUrl(source) {
    ...
    // Upload
    console.log(`Source file location is ${source}, and unique path for the file is ${path}`);
    await temporaryCloudStorage.upload(source, path);
    console.log(`${source} file is uploaded to cloud at path ${path}`)
    // Presign url
    const presignUrl = temporaryCloudStorage.generatePresignURL(path, "rd");
    console.log(`Generate PreSigned Url for ${source} is ${presignUrl}`);
    return presignUrl; 
};
