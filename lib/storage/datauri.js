...
const { GenericError, SourceUnsupportedError } = require('@adobe/asset-compute-commons');
const { actionName } = require('../action');
const { TemporaryCloudStorage } = require('./temporary-cloud-storage');
const { fileExistsAndIsNotEmpty } = require('../utils/fileutils');
...
async function getPreSignedUrl(source) {
    if(!fileExistsAndIsNotEmpty(source)){
        throw new SourceUnsupportedError(`Invalid or missing file ${source}`);
    }
    /**
