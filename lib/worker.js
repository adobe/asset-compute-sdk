...
class AssetComputeWorker {
    ...
    async processRendition(rendition, renditionCallback) {
        ...
        try {
            ...
            if (!this.options.disableRenditionUpload && !rendition.exists()) {
                console.error(`worker callback processing no rendition found at: ${rendition.path}`);
                throw new GenericError(`No rendition generated for ${rendition.id()}`, `${this.actionName}_process_norendition`);
            }

        } catch (err) {
            console.error(`worker callback processing processing failed with error after ${timer.durationSec()} seconds: ${err.message || err}`);
        ...
        if (this.shouldPostProcess(rendition)) {
            ...
            await imagePostProcess(rendition);
            this.timers.postProcessing += postProcessingTimer.duration(); //2 new timers
        }

        this.timers.processing += timer.duration() + (this.timers.postProcessing || 0);
    ...
    async renditionSuccess(rendition) {
        ...
        await this.metrics.sendMetrics(METRIC_RENDITION, {
            ...
            postProcessingDuration: this.timers.currentPostProcessing && this.timers.currentPostProcessing.duration(),
