...
class AssetComputeWorker {
    ...
    async processRendition(rendition, renditionCallback) {
        ...
        const postProcessingTimer = this.timers.currentPostProcessing;
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        // post-processing // TODO: Refactor this to share partas with post-processing in `processRendition` func
        for (const rendition of this.renditions) {
            const shouldDoPostProcess = await this.shouldPostProcess(rendition);
            if (shouldDoPostProcess) {
                try {
                    ...
                    console.log(`Post processing rendition ${rendition.name} from batch => ${postProcessedRendition.name}`);
                    ...
                    // TODO: Fix timers! postProcessingTimer.end();
                    // TODO: Fix timers! console.log(`Postprocessing finished in ${postProcessingTimer.durationSec()} seconds: ${rendition.name}`);
                } catch (err) {
                    // TODO: Fix timers! console.error(`rendition post-processing failed with error after ${postProcessingTimer.durationSec()}: ${err.message || err}`);
                    ...
                    console.error(`rendition post-processing for batch failed with error: ${err.message || err}`);
