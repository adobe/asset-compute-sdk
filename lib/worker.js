...
const defaultProcessingEngine = require('@adobe/asset-compute-image-processing');
...
class AssetComputeWorker {
    ...
    async processRendition(rendition, renditionCallback) {
        ...
        if (shouldDoPostProcess) {
            ...
            const postProcessingEngine = this.postProcessingEngine || defaultProcessingEngine;
    ...
    async shouldPostProcess(rendition) {
        // TODO: Base format check on mimetype (needs image update)
        ...
        let renditionExtension = path.extname(rendition.name).substring(1).toLowerCase();
