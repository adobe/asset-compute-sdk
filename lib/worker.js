...
const { Policy, BrokenCircuitError, SamplingBreaker } = require('cockatiel');
const breaker = Policy.handleAll().circuitBreaker(
    10 * 1000,
    new SamplingBreaker({ threshold: 0.5, duration: 60 * 1000 }),
);
class AssetComputeWorker {
    ...
    async postProcess(rendition) {
        ...
        try {
            if (await this.shouldPostProcess(rendition)) {
                ...
                const newRendition = new Rendition(rendition.originalInstructions, this.directories.postprocessing, rendition.index);
    ...
    async upload(rendition) {
        try {
            ...
            await putRendition(rendition, this.directories);
    ...
    async handleSendSuccessfulEventRequest(rendition, instructions, params) {
        try {
            return breaker.execute(async () => {
                await this.events.sendEvent(EVENT_RENDITION_CREATED, {
                    rendition: instructions,
                    metadata: await rendition.metadata(),
                    activationIds: params.customWorker ? [process.env.__OW_ACTIVATION_ID] : undefined,
    ...
    async handleSendFailureEventRequest(err, instructions, params) {
        try {
            return breaker.execute(async () => {
    ...
    async renditionSuccess(rendition) {
        ...
        console.log(`##### handleSendSuccessfulEventRequest`);
        await this.handleSendSuccessfulEventRequest(rendition, instructions, this.params);
    ...
    async renditionFailure(rendition, err, skipMetrics) {
        ...
        await this.handleSendFailureEventRequest(err, instructions, this.params);
