...
const { AssetComputeMetrics, AssetComputeEvents, ClientError, GenericError, Reason, OpenwhiskActionName } = require('@adobe/asset-compute-commons');
    ...
    async prepare() {
        ...
        if (!process.env.ASSET_COMPUTE_SDK_DISABLE_CGROUP_METRICS) {
            this.cgroupSampler = new Sampler(() => {
                ...
                const memoryUsage = metrics_object.memory.containerUsagePercentage;
                if (memoryUsage >= 90) {
                    if (!this.memoryUsageHigh) {
                        console.warn(`WARNING: Memory usage is very high: ${memoryUsage}%`);
                        this.memoryUsageHigh = true;
                    }
                } else {
                    this.memoryUsageHigh = false;
                }
        ...
        try {
            ...
            for (const rendition of this.renditions) {
                try {
                    ...
                    console.log("instructions for 'batchWorker()' callback:", Rendition.redactInstructions(rendition.instructions));
                } finally {
                    utils.setConsoleLogPrefix();
                }
            }
