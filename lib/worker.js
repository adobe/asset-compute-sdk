...
class AssetComputeWorker {
    ...
    async processRendition(rendition, renditionCallback) {
        ...
        console.log(`rendition generated in ${timer.duration()} seconds: ${rendition.name}, size = ${rendition.size()}`);
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        try {
            ...
            timer.end();
            console.log(`processing finished without error after ${timer.durationSec()} seconds`);
    ...
    async cleanup() {
        ...
        this.activationMetrics = Object.assign({}, cgroupMetrics || {}, {
        ...
        // send final metrics
        await this.metrics.sendMetrics('activation', this.activationMetrics);
