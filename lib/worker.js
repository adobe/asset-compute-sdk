...
const TIMEOUT_BUFFER = 6000; // time before an action timeout when to send timeout metrics
...
class AssetComputeWorker {
    ...
    timeUntilTimeout() {
        (process.env.__OW_DEADLINE - Date.now()) || DEFAULT_METRIC_TIMEOUT_MS;
    }
    ...
    sendEventsBeforeActionTimeout() {
        return setTimeout(
            async () => {
                ...
                // throw new Error('Aborting processing due to timeout');
            },
            this.timeUntilTimeout() - TIMEOUT_BUFFER
        );
    }
