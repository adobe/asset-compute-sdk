...
const { getSource, putRendition, getWatermark } = require('./storage');
    ...
    async postProcess(rendition) {
        ...
        try {
            if (await this.shouldPostProcess(rendition)) {
                ...
                if (rendition.instructions.watermark && rendition.instructions.watermark.watermarkContent !== undefined && rendition.instructions.watermark.watermarkContent !== null) {
                    const watermarkObject = Object.assign({
                        url: rendition.instructions.watermark.watermarkContent
                    }, rendition.instructions.watermark);
                    
                    rendition.instructions.watermark = await getSource(
                        watermarkObject,
                        this.directories.in,
                        this.options.disableSourceDownload,
                        'watermark'
            ...
            this.timers.postProcessing += postProcessingTimer.duration();
            console.error(`post-processing ${rendition.name} failed after ${postProcessingTimer.durationSec()}:`, err);
