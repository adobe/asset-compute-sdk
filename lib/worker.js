...
class AssetComputeWorker {
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        for (let rendition of this.renditions) {
            ...
            if (shouldDoPostProcess) {
                try {
                    await this.doPostProcess(rendition);
                    ...
                    await this.handleBatchPostProcessFailure(postProcessingTimer,err);
                    return;
                }
            } else {
    ...
    async handleBatchPostProcessFailure(postProcessingTimer,err) {
        console.error(`rendition post-processing for batch failed after ${postProcessingTimer.durationSec()}: ${err.message || err}`);
        await this.metrics.handleError(err, {
            location: `${this.actionName}_batchProcess`,
            metrics: {
                postProcessingDuration: this.timers.postProcessing + postProcessingTimer.duration(),
            }
        });
