...
class AssetComputeWorker {
    ...
    async processRendition(rendition, renditionCallback) {
        ...
        if (shouldDoPostProcess) {
            try {
                ...
                this.timers.postProcessing += postProcessingTimer.duration(); // add post processing rendition time to total post processing duration
                await this.renditionFailure(rendition, err);
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        this.timers.postProcessing = this.timers.currentPostProcessing = new Timer();
        let ppRendition = 0;
        for (let rendition of this.renditions) {
            ...
            const postProcessingTimer = new Timer();
            const shouldDoPostProcess = await this.shouldPostProcess(rendition);
            if (shouldDoPostProcess) {
                ppRendition++;
                try {
                    ...
                    console.error(`rendition post-processing for batch failed after ${this.timers.postProcessing.durationSec()}`);
                    console.error(`rendition post-processing for ${rendition.name} failed with error after ${postProcessingTimer.durationSec()}: ${err.message || err}`);
                    //for batch-rendition-metrics currentPostProcessing = postProcessing
                    // this.timers.postProcessing += postProcessingTimer.duration();
            ...
            // by convention if a even a single rendition is not postprocess eligible then failAll.
            // this block is only for reqeust with multiple rendition and if few are postprocess ineligible
            // fail fast catch the following:
            // if more than one rendition and one or more failed
            // skip: if only 1 rendition and is not post process eligible
            if(ppRendition > 0 && !shouldDoPostProcess) {
                console.error(`rendition post-processing for batch failed after ${this.timers.postProcessing.durationSec()}`);
        ...
        console.log(`Batch Postprocessing finished in ${this.timers.postProcessing.durationSec()} seconds`);
        //reset this.timers.postProcessing to 
        this.timers.postProcessing = this.timers.postProcessing.duration();
    ...
    async handleBatchPostProcessFailure(err) {
        console.error(`rendition post-processing for batch failed after ${this.timers.postProcessing.durationSec()}: ${err.message || err}`);
        await this.metrics.handleError(err, {
            location: `${this.actionName}_batchProcess`,
            metrics: {
                callbackProcessingDuration: this.timers.currentCallbackProcessing ? this.timers.currentCallbackProcessing.duration() : 0,
                // postProcessingDuration: this.timers.currentPostProcessing ? this.timers.currentPostProcessing.duration() : 0,
                postProcessingDuration: (this.timers.postProcessing ? this.timers.postProcessing.duration() : 0),
                processingDuration: (this.timers.currentCallbackProcessing ? this.timers.currentCallbackProcessing.duration() : 0) 
                                + (this.timers.postProcessing ? this.timers.postProcessing.duration() : 0)
            }
        });
