...
class AssetComputeWorker {
    ...
    async processRendition(rendition, renditionCallback) {
        ...
        try {
            ...
            if (! await fse.exists(rendition.path)) {
                throw new GenericError(`Processing returned without error after ${this.timers.currentProcessing.duration()} seconds, but no rendition was found at: ${rendition.path}`, `${actionName()}_processRendition`);
            }
    ...
    async cleanup() {
        ...
        // in case code fails before initiating metrics agent
        if (!this.metrics) {
            this.metrics  = new AssetComputeMetrics(this.params);
        }
