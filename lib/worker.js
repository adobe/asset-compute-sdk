...
const { imagePostProcess, needsImagePostProcess, prepareImagePostProcess } = require('./postprocessing/image');
...
class AssetComputeWorker {
    ...
    async compute(renditionCallback) {
        return this.run(async () => {
            for (const rendition of this.renditions) {
                try {
                    setConsoleLogPrefix(`[rendition ${rendition.index}]`);
    ...
    async processRendition(rendition, renditionCallback) {
        try {
            ...
            console.log("instructions for 'worker()' callback:", rendition.instructionsForEvent());
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        try {
            ...
            for (const rendition of this.renditions) {
                ...
                console.log("instructions for 'batchWorker()' callback:", rendition.instructionsForEvent());
                setConsoleLogPrefix();
            }
    ...
    async postProcess(rendition) {
        ...
        try {
            if (await this.shouldPostProcess(rendition)) {
                ...
                const newRendition = new Rendition(rendition.originalInstructions, this.directories.post, rendition.index);
