...
const { metrics: cgroupMetrics} = require('cgroup-metrics');
...
class AssetComputeWorker {
    ...
    async prepare() {
        ...
        this.cgroupSampler = new Sampler(() => {
            const metrics_object = cgroupMetrics(true);
        ...
        await this.cgroupSampler.start();
    ...
    async processRendition(rendition, renditionCallback) {
        ...
        try {
            ...
            if (! await fse.exists(rendition.path)) {
                throw new GenericError(`Processing returned without error after ${this.timers.currentProcessing.duration().toFixed(3)} seconds, but no rendition was found at: ${rendition.path}`, `${actionName()}_processRendition`);
            }
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        try {
            ...
            this.timers.currentProcessing.end();
            console.log(`renditions finished in ${this.timers.currentProcessing.duration().toFixed(3)} seconds`);
        ...
        for (const rendition of this.renditions) {
            ...
            if (await fse.exists(rendition.path)) {
                ...
                await this.renditionFailure(rendition, `No rendition generated for ${rendition.id()}`);
            }
        }
    }
    ...
    async cleanup() {
        ...
        if(this.cgroupSampler) {
            cgroupMetrics = await this.cgroupSampler.finish();
        }
        if (this.timers) {
