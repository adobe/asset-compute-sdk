...
class AssetComputeWorker {
    ...
    async processRendition(rendition, renditionCallback) {
        try {
            ...
            this.timers.processingCallback.stop();
            console.error(`worker() callback processing failed with error after ${this.timers.processingCallback.current()} seconds: ${err.message || err}`);
        ...
        console.log(`worker() callback generated rendition in ${this.timers.processingCallback.current()} seconds: ${rendition.name}`);
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        try {
            ...
            this.timers.processingCallback.stop();
            console.log(`processing finished successfully after ${this.timers.processingCallback.current()} seconds`);

        } catch (err) {
            this.timers.processingCallback.stop();
            console.error(`processing failed with error after ${this.timers.processingCallback.current()} seconds: ${err.message || err}`);
    ...
    async postProcess(rendition) {
        ...
        try {
            if (await this.shouldPostProcess(rendition)) {
                ...
                await imagePostProcess(rendition, newRendition.path);

                this.timers.postProcessing.stop();
                console.log(`post-processing ${rendition.name} finished successfully in ${this.timers.postProcessing.current()} seconds`);
            ...
            this.timers.postProcessing.stop();
            console.error(`post-processing ${rendition.name} failed after ${this.timers.postProcessing.current()}:`, err);
