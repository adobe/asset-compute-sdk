    ...
    async processRendition(rendition, renditionCallback) {
        ...
        try {
            ...
            if (! await fse.exists(rendition.path)) {
                console.error(`no rendition was found at: ${rendition.path}`);
                throw new GenericError(`No rendition generated for ${rendition.id()}`, `${actionName()}_processRendition`);
            }
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        for (const rendition of this.renditions) {
            ...
            if (await fse.exists(rendition.path)) {
                ...
                await this.renditionFailure(rendition, new GenericError(`No rendition generated for ${rendition.id()}`, `${actionName()}_batchProcessRendition`));
            }
        }
    }
