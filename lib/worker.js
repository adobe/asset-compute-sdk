...
class AssetComputeWorker {
    ...
    async batchProcessRenditions(renditionsCallback) {
        ...
        // batch post-process is all or none. so using currentPostProcessing(is timer) as postProcessing(is duration)
        this.timers.currentPostProcessing = new Timer();
        let ppRendition = 0;
        for (let rendition of this.renditions) {
            // start post-processing timer - used for logging current interation
            ...
            if (shouldDoPostProcess) {
                ...
                try {
                    ...
                    console.error(`rendition post-processing for batch failed after ${this.timers.currentPostProcessing.durationSec()}`);
            ...
            // following block is only for reqeust with multiple rendition and if few are postprocess ineligible
            // skip: if only 1 rendition and is not post process eligible
            // fail fast: if more than one rendition and one or more failed
            if(ppRendition > 0 && !shouldDoPostProcess) {
                console.error(`rendition post-processing for batch failed after ${this.timers.currentPostProcessing.durationSec()}`);
                const err = new GenericError('All rendition in batch worker must be postprocess eligible');
                await this.handleBatchPostProcessFailure(err);
                return;
            }
        }
        console.log(`Batch Postprocessing finished in ${this.timers.currentPostProcessing.durationSec()} seconds`);
        //reset this.timers.postProcessing to 
        this.timers.postProcessing = this.timers.currentPostProcessing.duration();
