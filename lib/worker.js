...
const { createDirectories, cleanupDirectories } = require('@adobe/asset-compute-pipeline').Prepare;
const { Utils:utils, Rendition } = require('@adobe/asset-compute-pipeline');
...
const { getSource, putRendition } = require('@adobe/asset-compute-pipeline').Storage;
...
const IS_PIPELINE_RENDITION = false;
...
class AssetComputeWorker {
    ...
    async prepare() {
        ...
        this.directories = await createDirectories(folderName, baseDirectory, pipeline);

        this.renditions = Rendition.forEach(this.params.renditions, this.directories.out, pipeline);
