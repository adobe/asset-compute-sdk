...
class AssetComputeWorker {
    ...
    async processRendition(rendition, renditionCallback) {
        ...
        try {
            ...
            console.error(`worker() callback processing failed with error after ${timer.durationSec()} seconds: ${err.message || err}`);
    ...
    async renditionSuccess(rendition) {
        ...
        await this.metrics.sendMetrics(METRIC_RENDITION, {
            ...
            postProcessingDuration: this.timers.currentPostProcessing.duration(),
            processingDuration: this.timers.currentCallbackProcessing.duration() + this.timers.currentPostProcessing.duration(),
