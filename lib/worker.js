    ...
    sendEventsBeforeActionTimeout() {
        return setTimeout(
            async () => {
                ...
                // hack to use the actual newRelic class to send metrics immediately
                clearTimeout(this.metrics.newRelic.actionTimeoutId);
                await this.metrics.newRelic.send('timeout', {
