...
const TIMEOUT_BUFFER = 5000; // time before an action timeout when to send timeout metrics
...
class AssetComputeWorker {
    ...
    sendEventsBeforeActionTimeout() {
        return setTimeout(
            async () => {
                console.log(`Action is about to timeout in ${timeUntilTimeout()}ms. Sending Adobe I/O events before timeout.`);
                ...
                await this.metrics.handleError(new GenericError(errorMessage), {
                    location: `${this.actionName}_timeout`,
                    metrics: {
