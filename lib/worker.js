    ...
    async processRendition(rendition, renditionCallback) {
        ...
        if (shouldDoPostProcess) {
            try {
                ...
                await imgProcessingEngine.imageProcess(rendition.path, postProcessedRendition.path, rendition.instructions);
                console.log('POST PROCESSED RENDITION:', Buffer.from(await fs.readFile(postProcessedRendition.path)).toString('base64').length);
