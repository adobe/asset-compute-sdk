...
async function getWatermark(paramsWatermark, inDirectory, disableWatermarkDownload) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        // local file support for `Asset Compute test-worker` unit tests
        // not supported for clients in production
        if (!isValidLocalFile(paramsWatermark.watermarkContent, inDirectory)) {
            throw new Error(`Invalid or missing local file ${paramsWatermark.watermarkContent}`);
        }
        return new Watermark(paramsWatermark.watermarkContent, inDirectory);
