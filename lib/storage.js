...
async function getSource(paramsSource, inDirectory, disableSourceDownload=false, basename) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        ...
        if (source && source.isWatermark && path.extname(source.name).toLowerCase() !== ".png" && !source.url.startsWith("data:image/png")) {
            throw new RenditionFormatUnsupportedError(`Invalid watermark format ${source.url}`);
        }
        if (disableSourceDownload) {
            ...
            if (protocol === DATA_PROTOCOL) {
                await datauri.download(source, source.path);
            } else {
                await http.download(source, source.path);
            }
        }

        if (source && source.isWatermark && isValidLocalFile(source.path)) {
            throw new Error(`Invalid or missing local file ${source.path}`);
        }
