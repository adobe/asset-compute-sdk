...
async function putRendition(rendition, directories) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        if (rendition.directory === directories.postprocessing) {
            ...
            // if we copy, we run into issues on Travis CI: 'EACCES: permission denied, unlink' on the host/test-worker
            // so we move first, then copy back (since we still need it at the original location)
            await fs.move(rendition.path, newRendition.path, { overwrite: true });
            // await fs.copyFileSync(newRendition.path, rendition.path);
