...
async function getSource(paramsSource, inDirectory, disableSourceDownload){
    ...
    if (process.env.NUI_UNIT_TEST_MODE) {
        ...
        if (disableSourceDownload) {
            ...
            throw new SourceUnsupportedError(`Invalid Https Url: ${paramsSource.url}`);
        } else {
            // TODO: logging
...
async function putRendition(rendition) {
    if (process.env.NUI_UNIT_TEST_MODE) {
        ...
        const msg = `Invalid Https Url: ${typeof rendition.target === "object"? rendition.target[0]: rendition.target}`;
