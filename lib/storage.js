...
async function putRendition(rendition, directories) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        if (rendition.directory === directories.postprocessing) {
            ...
            // HACK to prevent 'EACCES: permission denied, unlink' on Travis/CircleCI native linux environments
            // moving works, but direct copy does not, so we first copy locally then move
            const tempFile = path.join(rendition.directory, "temp-rendition");
            await fs.copy(rendition.path, tempFile);
            await fs.move(tempFile, newRendition.path);
        ...
        if (rendition.instructions.name) {
            ...
            // ensure it is readable by the host on Travis CI (no 'EACCES: permission denied, unlink')
            // await fs.chmod(newPath, 0o777);
            console.log(`WORKER_TEST_MODE: ${newPath}`, await fs.stat(newPath));
        }
