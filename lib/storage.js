...
const {Engine} = require('@adobe/asset-compute-pipeline');
    ...
    if (process.env.WORKER_TEST_MODE) {
        ...
        if (disableDownload) {
            ...
            if (protocol === DATA_PROTOCOL) {
                const preSignedUrl = await Engine.getPresignedUrl(source);
                console.log(`Base64 file uploaded to storgae and generated preSignedUrl ${preSignedUrl}`);
