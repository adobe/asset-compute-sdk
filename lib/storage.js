...
const { AssetComputeLogUtils, RenditionFormatUnsupportedError } = require('@adobe/asset-compute-commons');
...
/* TODO: refactor to check file type as a stream before downloading?
 * I'm not sure if this should be here for anything but watermark
 * or in http.js and datauri.js
 * https://www.npmjs.com/package/file-type
 */

function isValidLocalFile(file, basePath) {
    let fullPath;
    if(basePath && file) {
...
async function getSource(paramsSource, inDirectory, disableSourceDownload=false, basename) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        ...
        return new Source(paramsSource.url, inDirectory, basename);

    } else {
        const source = new Source(paramsSource, inDirectory, basename);
...
async function getWatermark(params, inDirectory, disableDownload) {
    // Note: validation has happened in validate.js before
    if (process.env.WORKER_TEST_MODE) {
        // local file support for `Asset Compute test-worker` unit tests
        // not supported for clients in production
        // params.watermarkContent will just be a filename like 'watermark.png'
        const filePath = path.join(inDirectory, params.watermarkContent);
        if (!fs.existsSync(filePath)) {
            throw new Error(`Invalid or missing local file ${params.watermarkContent}`);
        }
        return new Watermark(params, inDirectory);

    } else {
        const watermark = new Watermark(params, inDirectory);

        if (path.extname(watermark.name).toLowerCase() !== ".png" && !watermark.url.startsWith("data:image/png")) {
            throw new RenditionFormatUnsupportedError(`Invalid watermark format ${watermark.url}`);
        }

        if (disableDownload) {
            console.log(`Skipping watermark file download for ${AssetComputeLogUtils.redactUrl(params.watermarkContent)}`);
        } else {

            const protocol = URL.parse(params.watermarkContent).protocol;
            if (protocol === DATA_PROTOCOL) {
                await datauri.download(watermark, watermark.path);
            } else {
                await http.download(watermark, watermark.path);
            }
        }

        if (isValidLocalFile(watermark.path)) {
            throw new Error(`Invalid or missing local file ${watermark.path}`);
        }
