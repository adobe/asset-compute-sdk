...
async function getSource(paramsSource, inDirectory, disableSourceDownload){
    ...
    if (process.env.NUI_UNIT_TEST_MODE) {
        ...
        if (! await isValidLocalFile(paramsSource.url)) {
        ...
        if (disableSourceDownload) {
            ...
            throw new GenericError(`Invalid Https Url: ${paramsSource.url}`, `${actionName()}_getSource`);
        } else {
            // TODO: logging
...
async function putRendition(rendition) {
    if (process.env.NUI_UNIT_TEST_MODE) {
        // do nothing
    } else if (!checkUrls(rendition.target)) {
        throw new Error(`Invalid Https Url: ${typeof rendition.target === "object"? rendition.target[0]: rendition.target}`);
    } else {
        // TODO: https enforcement? https://git.corp.adobe.com/nui/asset-compute-sdk/issues/15
