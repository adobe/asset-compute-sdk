...
async function putRendition(rendition, directories) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        if (rendition.directory === directories.postprocessing) {
            ...
            // TODO: analyze and prevent 'EACCES: permission denied, unlink' on Travis/CircleCI native linux environments
            await fs.chmod(directories.postprocessing, 0o777);
            console.log(`WORKER_TEST_MODE stats for ${rendition.directory}:`, await fs.stat(rendition.directory));
