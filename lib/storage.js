...
async function putRendition(rendition, directories) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        if (rendition.directory === directories.postprocessing) {
            // move the post processing rendition to the rendition path as expected by "aio asset-compute test-worker"
            ...
            await fs.move(rendition.path, newRendition.path);
            // ensure it is readable by the host on Travis CI (no 'EACCES: permission denied, unlink')
            // await fs.chmod(newRendition.path, 0o777);
            console.log(`WORKER_TEST_MODE: ${newRendition.path}`, await fs.stat(newRendition.path));
