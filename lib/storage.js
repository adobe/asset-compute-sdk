...
async function getSource(paramsSource, inDirectory, disableSourceDownload) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        ...
        if (disableSourceDownload) {
            ...
            // TODO: error handling: errors caught & thrown in http.download() and then caught again in worker.js ???
...
async function getWatermark(paramsWatermark, inDirectory, disableWatermarkDownload) {
    // Note: validation has happened in validate.js before

    const watermark = new Watermark(paramsWatermark, inDirectory);

    if (path.extname(watermark.name).toLowerCase() !== ".png" && !watermark.url.startsWith("data:")) {
        throw new RenditionFormatUnsupportedError(`Invalid watermark format ${watermark.url}`);
    }

    if (disableWatermarkDownload) {
        console.log(`Skipping watermark file download for ${AssetComputeLogUtils.redactUrl(paramsWatermark.watermarkContent)}`);
    } else {

        const protocol = URL.parse(paramsWatermark.watermarkContent).protocol;
        if (protocol === DATA_PROTOCOL) {
    ...
    watermark.path = "/Users/dhendric/working/ADOBE/asset-compute-sdk/test/files/watermark.png"
    if (fs.pathExistsSync(watermark.path)) {
        console.log("File exists")
    }
    console.log(gm(watermark.path).identify)
    const assetType = await fileType.fromFile(watermark.path);
