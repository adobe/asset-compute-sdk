...
async function getMimeInformationFromCommand(rendition){
    let mime;
    let encoding;

    if(rendition.exists() && rendition.size() > 0){
        const {stdout} = await exec(`file -b --mime "${rendition.path}"`);
        const mimetype = stdout.trim();

        const mimeInfo = mimetype.split("; ");
...
class Rendition {
    constructor(instructions, directory, index = 0) {
        ...
        this._mime = null;
        this._encoding = null;
        this.mimeInfoPath = null; // mime info set by shellscript workers (js worker could also use this)
    }
    ...
    async setMimeInformation(mime=null, encoding=null){
        ...
        // shellscript mostly ------------------------------------------------------
        console.log('=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+');
        console.log('Trying to get info from mimefile...');
        console.log(this.mimeInfoPath);
        

        if(this.mimeInfoPath && await fs.pathExists(this.mimeInfoPath)){ // use the path containing the file with mime information
            let mimeInfoContent = await fs.readFile(this.mimeInfoPath);
            mimeInfoContent = mimeInfoContent.toString();
            console.log('Mime content:', mimeInfoContent);
            const mimeInfo = mimeInfoContent.split(";");

            if(mimeInfo.length === 2){ // we got something in an expected format
                this._mime = this._mime || mimeInfo[0].trim() || DEFAULT_CONTENT_TYPE;
                this._encoding = this._encoding || mimeInfo[1].trim();
            }
        } 
        console.log('this._mime:', this._mime);
        console.log('this._encoding:', this._encoding);
        console.log('/=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+');
        // fallback if things are missing ------------------------------------------------------
        if(!this._encoding || !this._mime){ // fallback to using file-type lib (won't set encoding)
            try{
                await commandExists('file'); // throws if `file` is not found
                const fileCmdResult = await getMimeInformationFromCommand(this);
                this._mime = this._mime || fileCmdResult.mime;
                this._encoding = this._encoding || fileCmdResult.encoding;
            } catch(err){
                console.log('`file` command not found', err); // err should be empty
                console.log('Trying to find mimetype using rendition file');
                const filetypeInfo = await FileType.fromFile(this.path);
                if(filetypeInfo) { // is only set if something was recognized
    ...
    async metadata() {
        ...
        try {
            ...
            console.log("could not determine mimetype (falling back to default):", DEFAULT_CONTENT_TYPE);
