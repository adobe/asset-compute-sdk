...
async function getMimeInformationFromFileCommand(rendition) {
    let mime;
    let encoding;
    ...
    try {
        ...
        mime = parsedContentType.type || DEFAULT_MIME_TYPE;
        encoding = parsedContentType.parameters.charset;
    } catch(err) {
        console.log('`file` command did not return a valid content type:', err.message);
        // will also happen in unit-tests if an in-memory or mocked filesystem is used (file can't see it properly)
    }
...
async function detectContentType(rendition) {
    ...
    try { // `file` command
        // assumes rendition exists and size > 0 (to be checked in caller)
        const fileCmdResult = await getMimeInformationFromFileCommand(rendition);
