...
const { detectContentType } = require('@adobe/asset-compute-pipeline').Sdk.Utils;
...
const Metadata = {
    ...
    TIFF_IMAGEHEIGHT: 'tiff:imageHeight' // TODO: should be TIFF_IMAGELENGTH: 'tiff:imageLength' 
};
...
class Rendition {
    ...
    async metadata() {
        ...
        try {
            ...
            console.log(`Error trying to find dimensions, no dimensions found:`, err.message || err);
        }

        // ensure we don't send null/undefined/non-finite values
        if(meta[Metadata.TIFF_IMAGEWIDTH] === null 
            || meta[Metadata.TIFF_IMAGEWIDTH] === undefined
            || !Number.isFinite(meta[Metadata.TIFF_IMAGEWIDTH])){
            console.log(`no width found for rendition of type ${meta[Metadata.DC_FORMAT]}`);
            delete meta[Metadata.TIFF_IMAGEWIDTH];
        }
        if(meta[Metadata.TIFF_IMAGEHEIGHT] === null 
            || meta[Metadata.TIFF_IMAGEHEIGHT] === undefined
            || !Number.isFinite(meta[Metadata.TIFF_IMAGEHEIGHT])){
            console.log(`no height found for rendition of type ${meta[Metadata.DC_FORMAT]}`);
            delete meta[Metadata.TIFF_IMAGEHEIGHT];
        }
    ...
    changeInstructions(newInstructions) {
        ...
        Object.freeze(this._originalInstructions);
    }
