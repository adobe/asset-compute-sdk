...
async function detectContentType(renditionPath) {
    let localMimeType;
    let localEncoding;

    try { // `file` command
        // assumes rendition exists and size > 0 (to be checked in caller)
        const fileCmdResult = await getContentTypeInformation(renditionPath);
        ...
        const filetypeInfo = await FileType.fromFile(renditionPath);
 ...
 */
class Rendition {
    constructor(instructions, directory, index = 0, name) {
        this.instructions = instructions; // e.g. quality, dpi, format etc
        this.directory = directory;
        this.name = name || Rendition.renditionFilename(instructions.fmt, index); // only base name
    ...
    async _detectContentType() {
        if (this.exists() && this.size() > 0) {
            const contentType = await detectContentType(this.path);
    ...
    exists() { // see and read the file
        console.log('## Looking for file:', this.path);
    ...
    static forEach(renditions, outDirectory) {
        return renditions.map((instructions, index) => {
            const name = Rendition.renditionFilename(instructions.fmt, index);
            return new Rendition(instructions, outDirectory, index, name);
        });
    }
}
