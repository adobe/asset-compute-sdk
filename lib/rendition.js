...
class Rendition {
    ...
    async _getMimeInformationFromCommand(){
        if(this.exists() && this.size() > 0){
            ...
            if((!this._mime || this._mime === DEFAULT_CONTENT_TYPE) && mimeInfo[0]){
                // file found more information and can replace default value
                this._mime = mimeInfo[0] || DEFAULT_CONTENT_TYPE;
            }
    ...
    async mimeType() {
        if(!this._mime){
            await this._getMimeInformationFromCommand();
        }
        return this._mime;
    }
