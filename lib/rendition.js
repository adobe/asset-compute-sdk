...
class Rendition {
    constructor(instructions, directory, index=0){
        ...
        const encodingMagic = new Magic(mmm.MAGIC_MIME_ENCODING);
        const naiveEncodingDetector = util.promisify(encodingMagic.detectFile);
        this.encodingDetector = naiveEncodingDetector.bind(encodingMagic);
    }
    ...
    async mimeType() {
        return this.mimeTypeDetector(this.path) || DEFAULT_CONTENT_TYPE;
    }
    ...
    async charset() {
        return this.encodingDetector(this.path);
    }
