...
const IMAGEMAGICK = true;
const gm = require("gm").subClass({ imageMagick: IMAGEMAGICK });
const { execSync } = require("child_process");
const fs = require("fs-extra");
const path = require("path");

// default operations to apply to an image during image processing
...
// Watermarking
function processWatermark(img, watermarkInstructions) {
    console.log('Processing watermark...');
    const placement = watermarkInstructions.placement || 'Center';
    const visibilityPercentage = watermarkInstructions.visibilityPercentage || 100;

    let watermark = watermarkInstructions.watermarkContent;
    if(watermarkInstructions.isEmbedded){
        // watermark embedded as base 64
        console.log('Using embedded watermark');
        watermark = 'inline:'+watermarkInstructions.watermarkContent;
    }

    img.dissolve(visibilityPercentage).gravity(placement);
    img.composite(watermark);

    console.log('Processing watermark... Done!');
    return img;
}

function applyImageOperations(img, instructions, operations){
...
async function logImageMagickVersion() {
    const version = IMAGEMAGICK ? execSync("identify --version") : execSync("gm version");
...
async function imageProcess(infile, outfile, instructions /*, operations=null*/) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        // turning off watermarking for increamental post-processing
        const selectedOperations = DEFAULT_OPERATIONS;
        img = applyImageOperations(img, instructions, selectedOperations);
