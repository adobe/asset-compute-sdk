...
class AssetComputeWorkerPipeline {
    ...
    async compute(params){
        ...
        debug("Refined plan to create rendition: ", plan.toString());
    ...
    async computeAllAtOnce(params){
        debug("Batch going through pipeline - `computeAllAtOnce`")
        const engine = this.prepareEngine(params);
        debug("Created pipeline engine");

        // pass through params for events, metrics, other authorization, ...
        const plan = new Plan();
        debug("Created initial plan (batch)");
        ...
        if(!Array.isArray(renditions) || renditions.length === 1){
            debug("Only one rendition in the batch");
