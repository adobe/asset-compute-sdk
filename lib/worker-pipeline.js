...
class AssetComputeWorkerPipeline {
    ...
    async computeAllAtOnce(params){
        ...
        debug("computeAllAtOnce: Batch going through pipeline - `computeAllAtOnce`")
        const engine = this.prepareEngine(params);
        ...
        let renditions = params.renditions;
        if(!Array.isArray(renditions) || renditions.length === 1){
            debug("computeAllAtOnce: Only one rendition in the batch");
        ...
        debug("computeAllAtOnce: Preparing plan for rendition creation...");
        // TODO Build a "representative" rendition? Or take the first one?
        engine.refinePlan(plan, input, params.renditions[0]);
        debug("computeAllAtOnce: Refined plan to create rendition: ", plan.toString());

        debug("computeAllAtOnce: Running pipeline...");
        const pipelineOutput = await engine.run(plan);
        debug("computeAllAtOnce: Pipeline finished running");
