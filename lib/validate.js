...
const { GenericError, SourceUnsupportedError } = require('@nui/asset-compute-commons');
        ...
        if (!process.env.WORKER_TEST_MODE) {
            if (params.source.url.startsWith('data:')) {
                if (!validDataUrl(params.source.url)) {
                    throw new SourceUnsupportedError(`Invalid or missing data url ${params.source.url}`);
                }
            } else {
                if ((!isHttpsUri(params.source.url))) {
