...
const { GenericError, SourceUnsupportedError, SourceCorruptError } = require('@adobe/asset-compute-commons');
...
function validateParameters(params){
    ...
    if (params.source !== undefined && params.source !== null) {
        ...
        if (params.watermark && params.watermark.watermarkContent !== undefined && params.watermark.watermarkContent !== null) {
            validateWatermark(params.watermark);
        }
    }
...
function validateWatermark (paramsWatermark) {
    // Only validates that content is an HTTPS or data URL and file extension is PNG
    if (!process.env.WORKER_TEST_MODE) {

        if (paramsWatermark.watermarkContent.startsWith('data:')) {
            if (!validDataUrl(paramsWatermark.watermarkContent)) {
                throw new SourceCorruptError(`Invalid or missing data url ${paramsWatermark.watermarkContent}`);
            }
        } else if ((!isHttpsUri(paramsWatermark.watermarkContent))) {
            throw new SourceUnsupportedError(`Invalid or missing https url ${paramsWatermark.watermarkContent}`);
        }
    }
}
