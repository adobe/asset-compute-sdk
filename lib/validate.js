...
const { Action } = require('@adobe/asset-compute-pipeline');
const { isHttpsUri } = require('valid-url');
const validDataUrl = require('valid-data-url');

function validateParameters(params){
    const validateLocation = `${Action.name}_validate`;
...
function validateRendition(rendition, index, location){
    if(location === null || location === undefined) location = `${Action.name}_validate`;
