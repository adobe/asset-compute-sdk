...
class Dimensions {
    constructor(width, height) {
        ...
        if (width && (width.width || width.height)) {
            this.width = (typeof width.width === "number")? width.width: undefined;
            this.height = (typeof width.height === "number")? width.height: undefined;
        } else {
            this.width = (typeof width === "number")? width: undefined;
            this.height = (typeof height === "number")? height: undefined;
        }
    }

    scale(factor) {
        return new Dimensions(this.width * factor, this.height * factor, 1.0);
    }
