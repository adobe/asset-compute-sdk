...
async function applyOperations(img, intermediateRendition, rendition, directories) {
    ...
    // From https://imagemagick.org/script/color-management.php
    // ImageMagick supports color profiles, however, for images without a profile or a declaration of 
    // colorspace, ImageMagick assumes non-linear sRGB. Most image processing algorithms assume a linear 
    // colorspace, therefore it might be prudent to convert to linear color before certain image processing 
...
async function imagePostProcess(intermediateRendition, rendition, directories) {
    ...
    if (intermediateRendition._intermediateMetadata.format === "SVG") {
        ...
        img = await convertSvg(intermediateRendition.path, intermediateRendition.instructions);
    } else {
        img = await read(intermediateRendition.path);
...
async function convertSvg(path, instructions) {
    ...
    if (fmt === "jpg" || fmt === "jpeg" ) {
        image.background("#FFFFFF"); // Change tranparency to white 
    } else {
        image.background("none"); // Keep alpha background
    }
    image.flatten(); // forces the image on its background
    image.setFormat(fmt);
