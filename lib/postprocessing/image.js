...
function parseBackgroundColor(instructions){
  ...
  if(typeof inputBackgroundColor === 'string') {
    if(inputBackgroundColor.startsWith('#')
      && inputBackgroundColor.length === 7){
  ...
  if(Number.isInteger(inputBackgroundColor)
    ...
    throw new GenericError(`Invalid background color RGB values set in instructions: ${instructions.pdfbgcolor}`);
  }
}
