...
/*
Exiftool values: 1 = Horizontal (normal)
2 = Mirror horizontal
3 = Rotate 180
4 = Mirror vertical
...
function getImageMetadata(file) {
    return new Promise((resolve) => {
        Exif(file, (error, metadata) => {
            ...
            filteredMetdata.format = metadata.FileType; // needs to be toUpper
            filteredMetdata.mime = metadata.MIMEType;
            filteredMetdata.pageGeometry = {};
            filteredMetdata.pageGeometry.height = metadata.ImageHeight;
            filteredMetdata.pageGeometry.width = metadata.ImageWidth;
            filteredMetdata.orientation = metadata.Orientation;
            filteredMetdata.quality = metadata.Quality;
            filteredMetdata.interlace = metadata.Interlace;
            filteredMetdata.colorspace = metadata.ColorSpaceData || metadata.ColorSpace || metadata.ColorType;
