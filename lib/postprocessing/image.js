...
async function getImageMetadata(file) {
    try {
        // suffix attributes with `#` to disable human-readable parsing if exiftool not used with `-n` flag
        const attributes = [
            "-Orientation",
            "-FileType",
            "-ImageHeight",
        ...
        if(stderr){
            throw new GenericError(`Reading metadata from intermediate rendition failed: ${stderr}`);
        }
        ...
        filteredMetadata.format = metadata.FileType;
        ...
        filteredMetadata.quality = metadata.JPEGQualityEstimate;
...
async function needsImagePostProcess(rendition, source) {
    ...
    const metadata = rendition._intermediateMetadata;
...
function getImageSize(metadata) {
    ...
    */
    ...
    const CHANGE_ORIENTATION_LIST = [ 
        ...
        // Exif tool parsed values (the `-n` option keeps numbers)
