...
const FORMAT_SUPPORTS_EMBEDDED_COLOR_PROFILE = {
    "jpeg": true,
    "jpg": true,
    "png" : true,
    "tif" : true,
...
const FORMAT_NEEDS_EXPLICIT_COLOR_PROFILE = {
    "webp": true
}
...
async function prepareImagePostProcess(rendition, options) {
    ...
    if (instructions.fmt && isSupportedOutputFormat(instructions.fmt)) {
        ...
        if (!supportedWorkerFormats.includes(instructions.fmt)) {
            // TMN attach color profile call
            let colorProfile;
            if(FORMAT_SUPPORTS_EMBEDDED_COLOR_PROFILE[options.source.extension]
                && FORMAT_NEEDS_EXPLICIT_COLOR_PROFILE[rendition.instructions.fmt]){
                colorProfile = "##COLORPROFILE";
                console.log("ICC color profile to use", colorProfile);
            }
            ...
            if (workerName === "worker-flite") {
                rendition.instructions.colorProfile = rendition.instructions.colorProfile || colorProfile;
            ...
            return Object.assign({}, instructions, { fmt: newFormat, colorProfile: colorProfile });
        }
    }
...
/*
#####
To extract a color profile: 
convert /Users/mathern/Downloads/original_jpeg.jpeg /Users/mathern/Downloads/colorprofile.icc

To apply a profile, must be applied before the colorspace conversion
`convert /Users/mathern/Downloads/original_jpeg.jpeg -profile /Users/mathern/Downloads/original_jpeg.icc /Users/mathern/Downloads/webp_with_profile.webp`

Full command for webp conversion with color profile:
convert /Users/mathern/Desktop/git/@nui-workers/worker-flite/action/tests/webp-embedded-icc/file.jpeg -background 'rgba(0, 0, 0, 0.0)' -auto-orient -orient Undefined -define webp:lossless=false -profile /Users/mathern/Desktop/git/@nui-workers/worker-flite/action/tests/webp-embedded-icc/original_jpeg_embedded_profile.icc /Users/mathern/Desktop/git/@nui-workers/worker-flite/action/tests/webp-embedded-icc/rendition.webp 
*/
...
async function applyOutputProperties(img, instructions) {
    ...
    // color profiles must be applied before colorspace conversion

    // Most asset compute renditions are used in digital display context (web, screens), where RGB is required
    if (instructions.fmt === "webp" && instructions.colorProfile) {
        // imagemagick does not properly apply color profiles on webp
        // so, color profiles must be applied explicitly before colorspace conversions
        img.define("precision:highres-transform=true"); // force usage of floating point for better results
        img.in("-black-point-compensation"); // ensure "black" color from source and destination can be mapped
        img.profile(instructions.colorProfile);
    }
    img.colorspace('sRGB');
...
async function imagePostProcess(intermediateRendition, rendition, directories) {
    ...
    console.log('## Tmp post-processing files: ##################################')
    console.log(directories.postprocessing)
    console.log('####################################')
    ...
    console.log("## intermediateRendition ########################")
    console.log(intermediateRendition)
    console.log("##########################")
