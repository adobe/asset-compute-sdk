...
async function processWatermark(img, renditionInstructions, outdir) {
    ...
    // composite with crop, orient not supported
    // https://github.com/aheckmann/gm/issues/549
    // img.gravity(placement);
    // img.composite(resizedWatermarkPath);

    img.out(resizedWatermarkPath);
    img.gravity(placement);
    img.out('-composite');
...
async function applyOutputProperties(img, instructions, outdir) {
    ...
    // inject watermarking -compostie should come last 
...
async function imagePostProcess(rendition, outfile) {
    ...
    await applyOutputProperties(img, rendition.instructions, outdir);
