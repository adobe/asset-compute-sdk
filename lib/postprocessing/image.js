...
async function getWatemarkInfo(rendition, directories) {
    const instructions = rendition.instructions;
    const targetBoundingBox = new Dimensions(instructions.width, instructions.height);

    // TODO drop old watermark.widthPercent once no longer used by AEM
    const watermarkScale = instructions.watermark.scale || (instructions.watermark.widthPercent / 100.0) || 1.0;

    // determine image dimensions
    const imageSize = getImageSize(rendition.intermediateMetadata);

    // TODO drop old watermark.watermarkContent once no longer used by AEM
    const watermarkSource = instructions.watermark.image || instructions.watermark.watermarkContent;
    // download watermark
    const watermarkDir = directories.in;
    const watermarkName = 'watermark.png';
    const asset = await getAsset(watermarkSource, watermarkDir, watermarkName, false);
    ...
    return {
        width: watermarkTargetSize.width,
        height: watermarkTargetSize.height,
        path: asset.path
    };
}

async function applyOperations(img, rendition, directories) {
    const instructions = rendition.instructions;
    ...
    if (typeof instructions.watermark === 'object'){
        const watermark = await getWatemarkInfo(rendition, directories);
        img.draw([`gravity Center image Over 0,0 ${watermark.width},${watermark.height} "${watermark.path}"`]);
    }
}
...
// exported function
async function imagePostProcess(rendition, outfile, directories) {
    const img = await read(rendition.path);

    await applyOperations(img, rendition, directories);
