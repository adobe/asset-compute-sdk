...
async function imagePostProcess(intermediateRendition, rendition, directories) {
    ...
    if (intermediateRendition._intermediateMetadata.format === "SVG") {
        ...
        if(intermediateRendition._intermediateMetadata.alpha){
            ...
            img = handleTransparency(img, intermediateRendition.instructions.fmt);
        }
    }
...
function convertSvg(img, instructions) {
    ...
    // SVG background is transparent by default when converting
    handleTransparency(image, instructions.fmt);
...
function handleTransparency(image, fmt){
    if (FORMAT_SUPPORTS_TRANSPARENCY[fmt]) {
        image.background("rgba(0, 0, 0, 0.0)"); // Keep alpha background
    } else {
        image.background("rgb(255,255,255)"); // Change tranparency to white 
    }
    image.flatten();  // forces the image on its background to "commit" the background
