...
const { OpenwhiskActionName, GenericError } = require('@adobe/asset-compute-commons');

const actionName = new OpenwhiskActionName().name;
...
async function prepareImagePostProcess(rendition, options) {
    if (rendition.instructions.fmt && isSupportedOutputFormat(rendition.instructions)) {

        const supportedFormats =
            options.supportedRenditionFormats
            || WORKER_SUPPORTED_FMT[actionName]
            || DEFAULT_WORKER_SUPPORTED_FMT;

        // in case the worker cannot generate the final rendition format itself, we have to do it via post processing
        if (!supportedFormats.includes(rendition.instructions.fmt)) {
            ...
            if (!newFormat) {
                throw new GenericError(`Worker does not support generating a compatible image format. Requested: ${SUPPORTED_INPUT_FMT} Supported by worker: ${supportedFormats}`);
            }

            console.log(`post-processing: adjusted fmt for worker callback: '${rendition.instructions.fmt}' -> '${newFormat}'`);

            rendition._forceImagePostProcess = true;

            return Object.assign({}, rendition.instructions, { fmt: newFormat});
        }
    }
...
async function needsImagePostProcess(rendition) {
    ...
    // NOTE: already checked in prepareImagePostProcess() above, no need to check again?
...
async function applyWatemark(img, intermediateRendition, rendition, directories) {
    ...
    // download watermark
    if(!watermarkSource) {
        throw new GenericError(`post-processing: Missing watermark.image in instructions : ${JSON.stringify(instructions)}`);
    }
 ...
 * @param {*} intermediateRendition existing intermediate rendition created by the worker callback
 * @param {*} rendition final rendition to write to, including instructions
 * @param {*} directories working directories of the current activation
 */
