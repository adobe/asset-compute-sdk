...
async function prepareImagePostProcess(rendition, options) {
    if (rendition.instructions.fmt && isSupportedOutputFormat(rendition.instructions)) {
        ...
        const supportedRenditionFormats = options.supportedRenditionFormats || DEFAULT_WORKER_SUPPORTED_FMT;
        // in case the worker cannot generate the final rendition format itself, we have to do it via post processing
        if (!supportedRenditionFormats.includes(rendition.instructions.fmt)) {
            ...
            const newFormat = supportedRenditionFormats.find(fmt => SUPPORTED_INPUT_FMT.includes(fmt));
            if (!newFormat) {
                throw new GenericError(`Worker does not support generating a compatible image format. Requested: ${SUPPORTED_INPUT_FMT} Supported by worker: ${supportedRenditionFormats}`);
            }
