...
async function applyWatemark(img, rendition, directories) {
    const instructions = rendition.instructions;
    ...
    // download watermark
    if(!watermarkSource) {
        throw new GenericError(`post-processing: Missing watermark.image in instructions : ${JSON.stringify(rendition.instructions)}`);
    }
...
async function applyOperations(img, rendition, directories) {
    ...
    if (typeof instructions.watermark === 'object'){
        await applyWatemark(img, rendition, directories);
    }
}
