...
async function applyOutputProperties(img, instructions) {   
    if (instructions.fmt === "png") { // for reproducible png files
        img.define("png:exclude-chunks=date");
    } else if (instructions.fmt === "tif" 
        || instructions.fmt === "tiff") { // to keep same compression as initial file (e.g. for png -> tiff)
