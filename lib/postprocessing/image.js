...
async function extractColorProfile(img, rendition, directories) {
    const profilePath = path.join(directories.postProcess, "profile.icc");
...
async function applyOutputProperties(img, instructions) {
    ...
    if (instructions.colorProfile) {
        // Good profiles explanations: https://legacy.imagemagick.org/Usage/formats/#profiles
        // color profiles must be applied before colorspace conversions
        ...
        //img.in("-profile", instructions.colorProfile.profilePath);
        img.profile(instructions.colorProfile.profilePath);
    }
    img.colorspace('sRGB');
...
async function imagePostProcess(intermediateRendition, rendition, directories) {
    ...
    const colorProfile = await extractColorProfile(img, rendition, directories);
    if(colorProfile){
        rendition.instructions.colorProfile = colorProfile;
    }
