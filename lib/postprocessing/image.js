...
// minimal instructions which many workers can already handle themselves
// const BASIC_INSTRUCTIONS = new Set([
//     "fmt",
//     "width",
//     "height",
//     "quality"
...
// list of post processing instructions we handle
...
async function getImageMetadata(file) {
    try {
        // [1x1+0+0] is a trick to avoid reading all pixels into memory, improving time for small images
        const result = await gm(`${file}[1x1+0+0]`).write("json:-");
        const json = JSON.parse(result.toString());

        // return only the one file in the json
        return json[0].image;
    } catch (e) {
