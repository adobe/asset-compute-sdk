...
// Imagemagick values and Exiftool values don't overlap, so luckily we can list them both
const CHANGE_ORIENTATION_LIST = [ 
    /* Imagemagick values */
...
function getImageMetadata(file) {
    return new Promise((resolve) => {
        Exif(file, (error, metadata) => {
            if (error) {
                // log error but do not throw, fall back to using imagemagick postprocessing
                console.log('Failed exiftool cmd:', error);
                resolve(undefined);
            }
            ...
            // normalize to have nicer names (and same names as for imagemagick's metadata reader)
            filteredMetadata.format = metadata.FileType; // needs to be toUpper
            filteredMetadata.mime = metadata.MIMEType;
            filteredMetadata.pageGeometry = {};
            filteredMetadata.pageGeometry.height = metadata.ImageHeight;
            filteredMetadata.pageGeometry.width = metadata.ImageWidth;
            filteredMetadata.orientation = metadata.Orientation;
            // filteredMetadata.quality = metadata.Quality;
            // filteredMetadata.interlace = metadata.Interlace;
            // filteredMetadata.colorspace = metadata.ColorSpaceData || metadata.ColorSpace || metadata.ColorType;
            // filteredMetadata.xRes = metadata.XResolution;
            // filteredMetadata.yRes = metadata.YResolution;
            // filteredMetadata.resolutionUnit = metadata.ResolutionUnit;

            resolve(filteredMetdata);
        });
    });
}
...
async function needsImagePostProcess(rendition, source) {
    ...
    let metadata;
    try {
        rendition._intermediateMetadata = await getImageMetadata(rendition.path);
        metadata = rendition._intermediateMetadata;
    } catch (e) {
        throw new GenericError(`Reading metadata from intermediate rendition failed: ${e.message}`);
    } 

    if(!metadata){
        throw new GenericError(`No metadata read from intermediate rendition`);
    }
