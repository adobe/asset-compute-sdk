...
function normalizeMetadata(metadata){
    ...
    if(metadata.PhotometricInterpretation === 5 || metadata.PhotometricInterpretation === '5'){
        metadata.colorspace = "CMYK";
    } else if (metadata.ColorSpace === 1 || metadata.ColorSpace === '1') {
...
async function write(img, outfile) {
    ...
    console.log("imagemagick command (post-processing):", args.join(" "));
    ...
    try {
        ...
        console.log(`imagemagick command (post-processing) took ${new Date() - start} msec`);
    }
}
