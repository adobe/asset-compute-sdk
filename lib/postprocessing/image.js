...
async function applyOperations(img, rendition) {
    ...
    if (typeof instructions.watermark === 'object'){
        ...
        const imageSize = new Dimensions(rendition.intermediateMetadata.pageGeometry);

        // TODO download watermark
        // TODO drop old watermark.watermarkContent once no longer used by AEM
        // const watermarkImage = instructions.watermark.image || instructions.watermark.watermarkContent;
        // BEGIN - test hack
        ...
        fs.writeFileSync(watermarkPath, pngFileData);
        // END - test hack

        const watermarkSize = new Dimensions(await gm(watermarkPath).size());
        ...
        img.draw([`gravity Center image Over 0,0 ${watermarkTargetSize.width},${watermarkTargetSize.height} "${watermarkPath}"`]);
    }
}
