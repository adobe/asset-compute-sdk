...
async function write(img, outfile) {
    ...
    try {
        ...
        await getImageMetadata(outfile);
    }
}
...
async function imagePostProcess(intermediateRendition, rendition, directories) {
    ...
    // use cwebp when ICC properties are present to preserve them in rendition
    if (rendition.instructions.fmt === "webp" && intermediateRendition._intermediateMetadata && intermediateRendition._intermediateMetadata.ICCProfileName) {
        console.log('Using cwebp to convert images');
        return convertUsingCwebp(intermediateRendition, rendition);
    }
...
/**
 * Runs cwebp to convert images to webp
 * @param {Rendition} intermediateRendition existing intermediate rendition created by the worker callback
 * @param {Rendition} rendition final rendition to write to, including instructions
 */
async function convertUsingCwebp(intermediateRendition, rendition) {
    try {

        const command = `cwebp ${intermediateRendition.path} -metadata icc -o ${rendition.path}`;

        console.log(`cwebp command is: ${command}`);

        return exec(command);
    } catch (e) {
        console.log("cwebp command failed", e);
        throw new GenericError(`Converting intermediate rendition using cwebp failed: ${e.message}`);
    } 
}
