...
async function extractColorProfile(img, intermediateRendition, rendition, directories){
}
...
async function applyOutputProperties(img, instructions) {
    ...
    if (instructions.colorProfile) {
        ...
        img.define("precision:highres-transform=true");
    }
    img.colorspace('sRGB');
...
async function imagePostProcess(intermediateRendition, rendition, directories) {
    ...
    const colorProfile = await extractColorProfile(img, intermediateRendition, rendition, directories);
    if(colorProfile){
