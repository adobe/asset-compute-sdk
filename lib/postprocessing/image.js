...
const EXTRA_INSTRUCTIONS = new Set([
    ...
    "pdfbgcolor",
    "compressionLevel",
    // ASSETS-61593: custom-profile dimension cap. Presence of either forces post-processing
    // so the constraint is applied even when the worker already produced the target format/size.
    "maxWidth",
    "maxHeight"
]);
...
async function applyOperations(img, intermediateRendition, rendition, directories) {
    ...
    // ASSETS-61593: enforce an optional maximum dimension cap (custom processing profiles).
    // The ">" geometry flag shrinks only images larger than the box while preserving aspect ratio,
    // and is a no-op when the rendition already fits — so renditions are never upscaled. This chains
    // after any width/height resize above, so an explicit target is further constrained if it exceeds
    // the cap. docs: https://www.imagemagick.org/script/command-line-processing.php#geometry
    if (instructions.maxWidth || instructions.maxHeight) {
        img.resize(instructions.maxWidth || null, instructions.maxHeight || null, ">");
    }
