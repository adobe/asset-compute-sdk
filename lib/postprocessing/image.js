...
async function applyOperations(img, intermediateRendition, rendition, directories) {
    ...
    } else if (instructions.watermark === true) {
        throw new GenericError(`Watermark value is incorrect: should be an object, but is a boolean: ${JSON.stringify(instructions)}`, "image_post_process");
    }
}
