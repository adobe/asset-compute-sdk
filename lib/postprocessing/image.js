...
async function getImageMetadata(file) {
    try {
        ...
        // To get all metadata (even computed): `exiftool -n -json -ee -api RequestAll=3 ${file}`
        console.log(`Metadata (exiftool) command is: ${command}`);

        const { stdout } = await exec(command);
        let metadata = JSON.parse(stdout); // exiftool outputs the json as string
...
async function needsImagePostProcess(rendition, source) {
    ...
    console.log('~~~~~~~~~~@@@@@@@@@@@@@@@@')
    console.log('~~~~~~~~~~@@@@@@@@@@@@@@@@')
    console.log(rendition._intermediateMetadata)
    console.log('~~~~~~~~~~@@@@@@@@@@@@@@@@')
    console.log('~~~~~~~~~~@@@@@@@@@@@@@@@@')

    if(!metadata.FileType){
        // could not recognize filetype - most likely it is corrupt
        // TODO Define a new error type for this case
...
async function applyOperations(img, intermediateRendition, rendition, directories) {
    ...
    console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
    console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
    console.log(intermediateRendition._intermediateMetadata)
    console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
    console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
