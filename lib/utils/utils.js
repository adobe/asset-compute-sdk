...
const LEGACY_ARG_MAX = 131072;
const CMD_SIZE_LIMIT = getArgMax();
const VAR_PATH = "./in/vars";

function getArgMax() {
    try {
        const ARG_MAX = parseInt(spawnSync("getconf", ["ARG_MAX"]).stdout.toString()) / 4;
        if (ARG_MAX > LEGACY_ARG_MAX) {
            return ARG_MAX;
        } else {
