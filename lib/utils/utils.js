...
// This should be roughly similar to ARG_MAX, some systems is 131072 per argument 
// but we're going to be very conservative and cap the total size to be safe.
const CMD_SIZE_LIMIT = 8000;
 ...
 * @param {Object} vars 
 * @param {Number} sizeLimit Size limit to uphold
...
function limitVariableSizes(vars, sizeLimit = CMD_SIZE_LIMIT) {
    // First sort variables by serialized (string representation) size so we get the smallest variables out first
    const keys = Object.keys(vars).sort((a,b) => 
        String(vars[a]).length - String(vars[b]).length
    );
    const sizeLimitedVars = {};
    let totalSize = 0;
    for (const key of keys) {
        const newSize = totalSize + String(key).length + String(vars[key]).length;
        if (newSize >= sizeLimit) {
            // We have hit our limit and all variables must be stored in temp files now
            const tmpFile = tmp.fileSync();
            fs.writeSync(tmpFile.fd, vars[key]);
            fs.closeSync(tmpFile.fd);
            sizeLimitedVars[key] = tmpFile.name;
