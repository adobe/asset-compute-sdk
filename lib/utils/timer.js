...
class Timer {
    ...
    // returns a human readable representation of the seconds with 3 decimal preceision
    toString() {
        this.stop();
        return this._measured ? this._total.toFixed(3) : "???";
    }

    // returns a Timer with just the current duration
    // useful for toString() for just current duration
    current() {
        const t = new Timer();
        t._measured = true;
        t._current = this._current;
        t._total = this._current;
        return t;
    }
