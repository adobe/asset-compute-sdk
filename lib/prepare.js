...
async function createDirectories() {
    ...
    //         in/             <- in
    //         out/            <- out
    //         vars/           <- large variables, created as needed (if a variabel is over 128kb in size)
    ...
    if (directories && directories.base) {
        ...
        if (process.env.WORKER_TEST_MODE) {
            await fse.copy(directories.out, "/out", {
                ...
                filter: src => {
                    if (src.startsWith(directories.postprocessing)) {
                        return false;
                    }
        ...
        try {
            ...
            console.log(`Error while cleaning up work directories: ${err.message || err}`);
            return false;
        }
    }
    return true;
