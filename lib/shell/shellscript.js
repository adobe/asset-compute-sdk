...
class ShellScriptWorker {
    ...
    async processWithScript(source, rendition) {
        ...
        const errorFile = await prepareErrorFile(rendition.directory);
        rendition.mimeInfoPath = await prepareMimeInfoFile(rendition.directory, rendition.id());
