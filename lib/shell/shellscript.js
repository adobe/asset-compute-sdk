...
class ShellScriptWorker {
    ...
    async processWithScript(source, rendition) {
        ...
        const mimeFile = prepareMimeFile(rendition.directory);
        ...
        const env = setVariables(rendition, source, files);
...
function prepareInfoFile(directory, subdirectory, filename) {
    const infoDir = path.resolve(directory, subdirectory);
    fs.mkdirSync(infoDir, {recursive: true});
    return path.resolve(infoDir, filename);
}
...
function prepareErrorFile(directory) {
    return prepareInfoFile(directory, "errors", "error.json");
}

// example mime txt (as outputted by `file` command if available):
// text/plain; charset=us-ascii
// image/gif; charset=binary
function prepareMimeFile(directory) {
    return prepareInfoFile(directory, "mime", "mime.txt");
}
