...
class ShellScriptWorker {
    ...
    async processWithScript(source, rendition) {
        ...
        try {
            // send trace output to stdout instead of stderr
            env.BASH_XTRACEFD = "1";
...
function newScriptError(err, errorFile) {
    ...
    if (fs.existsSync(errorFile)) {
        ...
        /*
            example error json, mimicking the error:
            {
        ...
        */
        try {
            ...
            return new errors.GenericError(err.message || err, `${actionName()}_shellScript`);
        }
