...
class ShellScriptWorker {
    ...
    async processWithScript(source, rendition) {
        ...
        console.log('---------');
        console.log(rendition.instructions);
        console.log('---------');

        const errorFile = prepareErrorFile(rendition.directory);
        rendition.mimeInfoPath = prepareMimeInfoFile(rendition.directory, rendition.id());

        console.log('*** ---------------- Expected path for file with mimetype data: ');
        console.log(rendition.mimeInfoPath);
        console.log('/*** ---------------- ');

        // inherit environment variables
        const env = setVariables(rendition, source, errorFile, rendition.mimeInfoPath);

        console.log(' ***** ---- Expected env: ');
        console.log(env);
        console.log(' ***** ---- ');
        ...
        console.log('**** Rendition after sh-script: ');
...
function prepareMimeInfoFile(directory, renditionId) {
    const mimeDir = path.resolve(directory, "mimeinfo");
    fs.mkdirSync(mimeDir, {recursive: true});
    return path.resolve(mimeDir, `mimeinfo-${renditionId}.txt`);
}
...
function setVariables(rendition, source, errorFile, mimefile){
    // inherit environment variables
