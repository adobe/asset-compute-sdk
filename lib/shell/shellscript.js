...
class ShellScriptWorker {
    ...
    async processWithScript(source, rendition) {
        ...
        rendition.mimeInfoPath = preparedFiles.typeFile;
        ...
        try {
            ...
            if(await fs.pathExists(rendition.mimeInfoPath)){ // use the path containing the file with mime information
                let mimeInfoContent = await fs.readFile(rendition.mimeInfoPath);

                mimeInfoContent = mimeInfoContent.toString();

                const parsedContentType = contentType.parse(mimeInfoContent);
                rendition.setContentType(parsedContentType.type, parsedContentType.parameters.charset);
            } 
        } catch (err) {
...
async function prepareMetadata(directory, rendition) {
    // TODO: JIRA story to move errors content into a new `metadata` folder instead of `errors`
    ...
    // so no need for: const typeFile = path.resolve(mimeDir, `type-${rendition.id()}.txt`);
