  ...
  return function(Module) {
        ...
        while (idx < stream.getdents.length && pos + struct_size <= count) {
          ...
          if (name[0] === '.') {
            ...
            type = FS.isChrdev(child.mode)
              ? 2
              : FS.isDir(child.mode) ? 4 : FS.isLink(child.mode) ? 10 : 8;
          }
          tempI64 = [
            id >>> 0,
            ((tempDouble = id),
    ...
    function integerReadValueFromPointer(name, shift, signed) {
      switch (shift) {
        case 0:
          if (signed) {
            return function readS8FromPointer(pointer) {
              return HEAP8[pointer];
            };
          } else {
            return function readU8FromPointer(pointer) {
              return HEAPU8[pointer];
            };
          }
        case 1:
          if (signed) {
            return function readS16FromPointer(pointer) {
              return HEAP16[pointer >> 1];
            };
          } else {
            return function readU16FromPointer(pointer) {
              return HEAPU16[pointer >> 1];
            };
          }
        case 2:
          if (signed) {
            return function readS32FromPointer(pointer) {
              return HEAP32[pointer >> 2];
            };
          } else {
            return function readU32FromPointer(pointer) {
    ...
    function _fd_fdstat_get(fd, pbuf) {
      try {
        ...
        var type = stream.tty
          ? 2
          : FS.isDir(stream.mode) ? 3 : FS.isLink(stream.mode) ? 7 : 4;
        HEAP8[pbuf >> 0] = type;
        return 0;
      } catch (e) {
        if (typeof FS === 'undefined' || !(e instanceof FS.ErrnoError)) {
    ...
    var stackRestore;
    var stackAlloc;
    var stackSave = (stackRestore = stackAlloc = function() {
      abort(
    ...
    var HEAPF32;
    var HEAPF64;
    var STACK_BASE = 5292128;
    var STACK_MAX = 49248;
    var DYNAMIC_BASE = 5292128;
    var DYNAMICTOP_PTR = 49088;
    ...
    var wasmBinaryFile = 'magic-js.wasm';
    ...
    stringToUTF8('GMT', 49152, 4);
    var ___tm_timezone = 49152;
    var ENV = {};
    ...
    var asmLibraryArg = {
      ...
      __syscall60: ___syscall60,
      __syscall63: ___syscall63,
    ...
    if (!Object.getOwnPropertyDescriptor(Module, 'NODEFS')) {
      Module.NODEFS = function() {
        abort(
          "'NODEFS' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
        );
      };
    }
    if (!Object.getOwnPropertyDescriptor(Module, 'NODERAWFS')) {
      Module.NODERAWFS = function() {
        abort(
          "'NODERAWFS' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
        );
      };
    }
